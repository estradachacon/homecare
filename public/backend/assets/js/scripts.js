(function ($) {
	...
	$(document).on('click', '.btn-remove-href', function () {
		var link = $(this).attr('href');
		let title = $(this).data('title');
		let text = $(this).data('text');
		let confirmText = $(this).data('confirmtext');
	...
	$(document).on('click', '.href-remove', function () {
		var link = $(this).attr('href');
		let title = $(this).data('title');
		let text = $(this).data('text');
		let confirmText = $(this).data('confirmtext');
	...
	$(".trickycode-file").after("<input type='text' class='form-control filename' readOnly>"
		+ "<button type='button' class='btn btn-primary trickycode-upload-btn'>Seleccionar</button>");
	...
	$(document).on("click", ".ajax-modal", function () {
		...
		$.ajax({
			...
			beforeSend: function () {
				...
				// console.log(size);
				if (size != '') {
					$("#main_modal >.modal-dialog").addClass(size);
				}
				...
				if ($("#main_modal .select2-ajax").length) {
					$('.select2-ajax').each(function (i, obj) {
						...
						$(this).select2({
							...
							ajax: {
								url: _url + '/ajax/get_table_data?table=' + $(this).data('table') + '&value=' + $(this).data('value') + '&display=' + $(this).data('display') + display2 + where + items + order_note_id,
				...
				if ($("#secondary_modal .select2-ajax").length) {
					$('.select2-ajax').each(function (i, obj) {
						...
						$(this).select2({
							dropdownParent: parent,
							ajax: {
								url: _url + '/ajax/get_table_data?table=' + $(this).data('table') + '&value=' + $(this).data('value') + '&display=' + $(this).data('display') + display2 + where + order_note_id,
								delay: 250,
								dataType: 'json',
								data: function (params) {
									return {
			...
			beforeSend: function () {
				...
				if (json['result'] == "success") {
					...
					if (reload != false) {
						//Main Modal
						if (json['action'] == "store") {
							$('#main_modal .ajax-submit-noreload')[0].reset();
						}
						...
						//Secondary Modal
						if (json['action'] == "store") {
							$(current_modal).find('.ajax-submit-noreload')[0].reset();
						}
	...
	$(document).on('click', '.select-alert', function () {
		var link = $(this).attr('href');
		...
		let isRequiredInput = $(this).data('required');
		let hideinput = $(this).data('hideinput');
	...
	document.querySelectorAll('.color-picker').forEach(function (el, index) {
		...
		if (input_hiden) {
			pickr.on('save', (color) => {
				const hex = color.toHEXA().toString(); // obtiene color como #rrggbb
				document.getElementById(input_hiden).value = hex;
				pickr.hide(); // opcional: cerrar el picker despuÃ©s de elegir
			});
		}
	});
	document.querySelectorAll('.text-color-picker').forEach(function (el, index) {
		...
		const pickr = Pickr.create({
			...
			components: {
				// Solo permitir swatches, sin selector completo
				preview: true,
				opacity: false,
				hue: false,
...
$(document).on('click', '.tracking-alert', function () {
	...
	let confirmText = $(this).data('confirmtext');
	let placeholder = $(this).data('placeholder');
	let is_all_prepared = $(this).data('all-prepared');
	let is_all_verified = $(this).data('all-verified');
	// let isRequiredInput = $(this).data('required');
	// let hideinput = $(this).data('hideinput');
	...
	// 	inputAttributes.required = 'required'
	// }
	if (typeof is_all_verified !== 'undefined') {
		if (is_all_verified == 0) {
			// deben finalizar la preparacion de los productos
			Swal.fire({
				icon: 'warning',
				title: 'Falta verificaciÃ³n',
				text: 'La operaciÃ³n no puede continuar hasta que todos los productos estÃ©n marcados como verificados y seleccionados en la nota.',
	...
	if (typeof is_all_prepared !== 'undefined') {
		if (is_all_prepared == 0) {
			// deben finalizar la preparacion de los productos
			Swal.fire({
				icon: 'warning',
				title: 'Falta preparaciÃ³n',
				text: 'La operaciÃ³n no puede continuar hasta que todos los productos estÃ©n marcados como preparados y seleccionados en la nota.',
				confirmButtonText: 'Entendido',
				confirmButtonColor: '#3085d6'
			});
			return;
