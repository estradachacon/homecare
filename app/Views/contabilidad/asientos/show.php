        ...
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h4 class="mb-0">
                    ...
                    Asiento <strong>AST-<?= str_pad($asiento->numero_asiento, 5, '0', STR_PAD_LEFT) ?></strong>
                    <span class="badge bg-<?= $estadoBadge[$asiento->estado] ?> ms-2"><?= $asiento->estado ?></span>
                    <span class="badge bg-<?= $tipoBadge[$asiento->tipo] ?> ms-1"><?= $asiento->tipo ?></span>
                </h4>
                <div class="ms-auto d-flex gap-2">
                    <?php if ($asiento->estado === 'BORRADOR' && tienePermiso('aprobar_asiento')): ?>
                    <button class="btn btn-success btn-sm" onclick="aprobarAsiento(<?= $asiento->id ?>)">
                        <i class="fa-solid fa-check"></i> Aprobar
                    </button>
                    <?php endif; ?>
                    <?php if ($asiento->estado !== 'ANULADO' && tienePermiso('anular_asiento')): ?>
                    <button class="btn btn-danger btn-sm" onclick="anularAsiento(<?= $asiento->id ?>)">
...
<div class="modal fade" id="modalAnular" tabindex="-1">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Anular Asiento</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
function aprobarAsiento(id) {
    Swal.fire({ title:'¿Aprobar asiento?', text:'Esto actualizará los saldos contables.', icon:'question',
        showCancelButton:true, confirmButtonText:'Aprobar', cancelButtonText:'Cancelar'
    }).then(r=>{
        if(!r.isConfirmed) return;
        fetch(`<?= base_url('contabilidad/asientos/aprobar/') ?>${id}`, {method:'POST'})
...
function confirmarAnulacion() {
    ...
    if (!motivo) { Swal.fire('Requerido', 'Ingresa el motivo de anulación', 'warning'); return; }
    const form = new FormData(); form.append('motivo', motivo);
    fetch(`<?= base_url('contabilidad/asientos/anular/') ?>${asientoIdActual}`, {method:'POST', body:form})
        .then(r=>r.json()).then(d=>{
